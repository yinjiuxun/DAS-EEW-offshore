### This is the same as the iter_regression.py, but only used for the regression with peak amplitude from different time windows.
#%%
import sys
import pandas as pd
import numpy as np
...
regression_results_dir = results_output_dir + f'/iter_regression_results_smf{weight_text}_{min_channel}_channel_at_least_P_3s_S_6s'
...
peak_amplitude_df = peak_amplitude_df[['event_id', 'magnitude', 'depth_km', 'channel_id', 'distance_in_km', 'snrP', 'snrS', 'peak_P_3s', 'peak_S_6s', 'region']]
peak_amplitude_df = peak_amplitude_df.rename(columns={'peak_P_3s':'peak_P', 'peak_S_6s': 'peak_S'})
