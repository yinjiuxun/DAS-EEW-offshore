...
from utility.processing import filter_event_first_order, calculate_autocorrelation
...
for i_region in range(len(region_list)):
    ...
    for i_wavetype, wavetype in enumerate(site_term_list):
        ...
        percentile_20 = np.percentile(temp[f'site_term_{wavetype.upper()}'], 20)
        percentile_80 = np.percentile(temp[f'site_term_{wavetype.upper()}'], 80)
        ...
        gca.text(x=0, y=1, text=f'20\% percentile: {10**percentile_20:.2f}, 80\% percentile: {10**percentile_80:.2f}')
        if i_wavetype == 0:
