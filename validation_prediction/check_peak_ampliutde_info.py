...
# Filter events
peak_amplitude_df_sanriku = filter_event_first_order(
    peak_amplitude_df_sanriku,
    snr_threshold=snr_threshold_Sanriku,
...
# Event filtering
peak_amplitude_df_all = peak_amplitude_df_all[['event_id', 'magnitude', 'depth_km', 'channel_id', 'distance_in_km', 'snrP', 'snrS', 'peak_P', 'peak_S', 'region']]
...
for i_region, event_folder in enumerate(event_folder_list):
    DAS_info_files = event_folder + '/das_info.csv'
    catalog_file =  event_folder + '/catalog.csv'

    # Load DAS channel info
...
fig.show()
fig.savefig(figure_output_dir + '/map_of_earthquakes_CA_GMT_gray.png')
